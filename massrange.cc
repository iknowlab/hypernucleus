...
void MCScalc(const char *write_filename,double Range,double Step,double IZ,double RR,double D,double Mass,double Meas_Error){
	...
	if(writing_file.fail())readerr(write_filename);
		...
		for (Range=Range; Range<=EndRange; Range=Range+Step) {
			...
			double KE0 = 0.0;
			double dKE = 10.0;
			double Range_m = Range - (Step*2.0);
			if(Range_m<0){
				fprintf(stderr,"Imput Range Error!");
				exit(1);
			}//if
			KE = 0.0;
			double Ene[2];
			...
			if(Mass > 0.){
				...
				for(int t=0;t<2;t++){
					...
					Range_m = Range_m + (Step * 2.0 * (double)t);
				...
				/* root mean square ? */
//				KE=sqrt(Ene[0]*Ene[0]+Ene[1]*Ene[1]);
				KE=sqrt(Ene[0]*Ene[1]);

/* debug output */
				if(Range<1000&&Mass<130)std::cerr<<Ene[0]<<" "<<Ene[1]<<std::endl;
