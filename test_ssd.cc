...
double th_correct(double mass, double range){
	...
	if(range < 201){
		return 1.075*correct;
	}
	else if(range < 301)return 1.200;
	else if(range < 401)return 1.075;
	...
	if(correct>=1.0 && correct<1.3){
		correct *= (-0.917*correct+2.267);
	}
	else correct *=1.075;
...
//		return (double)g1/((g1+g2)*pow((1.+pow(u0_th,(double)1./d))-cos(th),d));
		return (double)g1/((g1+g2)*pow(1.-cos(th),d));
	}
}
