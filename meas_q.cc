...
/* .oOo.oOo.oOo.oOo.oOo.oOo.oOo. theta geant4 correction .oOo.oOo.oOo.oOo.oOo.oOo. */
double th_correct(double mass, double range){
	...
	if(correct < 1){
			if(range > 3950)return 1.35*1.15;
			else if(range <= 3950 && range > 1950)return 1.35*(range*0.000075+0.85);
			else return 1.35;
	}

	if(correct>=1.0 && correct<1.3)correct *= (-0.917*correct+2.267);
	else correct *= 1.075;	

	if(range > 3950 ) return correct*1.15;
	else if(range <= 3950 && range > 1950)return correct*(range*0.000075+0.85);
	else return correct;
...
//		double step = Read_Param("Step");
//		double range = Read_Param("Meas._Err");
		...
		for(int j=0; j<10000; j++){
			th = dist(gen);
			double dx = (2.* m_step)*tan((double)th/sqrt(3.));
			dx /= 2.;
...
//			data[j][i].theta = data[j][i].ssdx * (double)sqrt(3)/step;
			data[j][i].theta = sqrt(3.) * atan( data[j][i].ssdx / (double)step );

/* .oOo.oOo.oOo.oOo.oOo.oOo.oOo. theta geant4 correction .oOo.oOo.oOo.oOo.oOo.oOo. */
//			data[j][i].theta *= th_correct(data[j][i].mass,data[j][i].range);
/* .oOo.oOo.oOo.oOo.oOo.oOo.oOo. theta geant4 correction .oOo.oOo.oOo.oOo.oOo.oOo. */
