...
public:
	...
	void Read(){
		TGraph *g[Argc()-1];
		TGraphErrors *ge[Argc()-1];
		int k;
		char buf[24];
		double measerr;

		cerr << "imput meas error\t";
		cin >> buf;
		measerr=atof(buf);
...
//				g[k-1] -> TGraph::SetPoint(j,range,dx);
				ge[k-1]-> TGraphErrors::SetPoint(j, range, dx);
				ge[k-1]-> TGraphErrors::SetPointError(j,0,measerr);
			...
			ge[k-1]->SetTitle(Argv(k));
			ge[k-1]->SetMarkerStyle(21);
			ge[k-1]->SetMarkerSize(0.6);
			ge[k-1]->SetLineWidth(1);
			ge[k-1]->SetMarkerColor(MyPalette[k*NCont/Argc()]);
//			ge[k-1]->SetLineColor(1);
			ge[k-1]->SetLineColor(MyPalette[k*NCont/Argc()]);
